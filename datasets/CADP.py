...
class CADP(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        output = self.normalize(torch.stack(image_list))
...
class CADP_Demo(Dataset):
    def __init__(self, data_dir, data_partition, clip_len = 30, sample_stride=1, num_workers=24, image_size=224, norm_value=255,
                multiscale_crop=4, temporal_stride=1):
        super(CADP_Demo, self).__init__()
        self.video_path = Path(data_dir)
        self.data_partition = data_partition    
        self.clip_len = clip_len
        self.image_size = image_size
        ...
        i = 0
        #Read data path
        #vid_nn = os.path(self.video_path)
        vid_list = glob(data_dir  + '/*')
        #pkl_list = glob(vid_list + '/*')   
        #self.video_list.extend(pkl_list)
        #self.labels.extend([i]*len(pkl_list))
                
        print('demo video = %i'%(len(vid_list)))
    ...
    def __getitem__(self, idx):
        ...
        for image_ in data:
            for j in range(len(image_list) - 30):
                pdb.set_trace()
                image_list.append(torch.from_numpy(image_[j:j+31].transpose(-1,0,1).copy()))
        pdb.set_trace()
        output = self.normalize(torch.stack(image_list))
        #print(torch.max(output))
        #print(output.type()) # torch. DoubleTensor, 이전꺼는 torch.ByteTensor
