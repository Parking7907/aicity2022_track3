...
class AICity_Demo(Dataset):
    def __init__(self, data_dir, data_partition, clip_len = 30, sample_stride=1, num_workers=24, image_size=224, norm_value=255,
                multiscale_crop=4, temporal_stride=1):
        super(AICity_Demo, self).__init__()
        self.video_path = data_dir
        self.data_partition = data_partition    
        self.clip_len = clip_len
        self.image_size = image_size
        self.norm_value = norm_value
        ...
        print("Test Video Done : %i"%(len(self.video_list)))
...
#train_dataset = AICity_Demo(config['datasets']['train'], 'train', config['MYNET']['sequence_size'], temporal_stride=config['datasets']['stride'])
#pdb.set_trace()