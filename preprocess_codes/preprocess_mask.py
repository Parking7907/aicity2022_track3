...
mask_path1 = "/home/data/aicity/u2net_mask/"
mask_path2 = "/home/data/aicity/u2netp_mask/"
...
for label_path in label_list:
    ...
    for i, filename in enumerate(File_list):
        ...
        if filename == ' ':
            ...
            mask_dir1 = mask_path1 + dir_n + '/' + filename + '/'
            mask_dir2 = mask_path1 + dir_n + '/' + filename + '/'
            print(frame_dir, mask_dir1, mask_dir2)
            if time_st == 0:
                print("Wow!", i)
                end_time = -1
        ...
        if (end_time + 1) == time_st:
            ...
            if end_time - time_st > 1:
                ...
                for j in range(frame_st, frame_en): # Frame_en은 제외
                    ...
                    mask_n1 = mask_dir1 + "%06d.jpg"%j
                    mask_n2 = mask_dir2 + "%06d.jpg"%j
                    try:
                        fr = Image.open(frame_n)
                        mk1 = Image.open(mask_n1)
                        mk2 = Image.open(mask_n2)
                        fr = np.array(fr)
                        mk1 = np.array(mk1)
                        mk2 = np.array(mk2)
                    except:
